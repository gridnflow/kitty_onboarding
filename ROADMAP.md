...
| 파일 | 내용 |
|---|---|
| `index.html` | HTML 셸 (HUD 마크업 + 스크립트 로드 순서만) |
| `style.css` | 전체 UI 스타일 |
| `js/data.js` | 콘텐츠 데이터: `NPCS`(온보딩 지식 대사), `OBJECTIVES`, `INTERACTS`, `FINAL_QUIZ` — 콘텐츠 수정은 대부분 여기서 끝남 |
| `js/engine.js` | 렌더러/카메라/환경맵, 재질(`MAT`), 캔버스 텍스처, 지오메트리 헬퍼, Kenney 펫 로더(`makePet`) |
| `js/world.js` | 씬 구성: 로비+바깥 풍경, 사무실(12층), 전역 조명, 플레이어/마커 |
| `js/game.js` | 게임 로직: 스테이지 머신, `rideSequence`(엘베 상승/하강 공용), 온보딩 위크, 입력, 충돌, 메인 루프 |
...
**주의**: 모두 클래식 `<script>`(모듈 아님)이며 **로드 순서 고정**: three → GLTFLoader → kenney-assets → data → engine → world → game. 각 파일의 최상위 `const/let`은 뒤에 로드되는 파일에서 그대로 보인다(전역 렉시컬 스코프 공유). 게임 내 텍스트는 전부 영어(2026-07-05 영어화 완료).
...
- `?yaw=3.14` — 초기 카메라 각도
- `?autoride` — 엘베 상승 자동 실행(+`&office`면 하강). 컷신이 고속화됨(`window.RIDE_FAST`)
- 저장 리셋: DAY 요약 화면에서 R, 또는 콘솔에서 `localStorage.clear()`

### 엘베 시퀀스 검증 팁
헤드리스 가상시간에서는 rAF 의존 컷신이 끝까지 진행되지 않는다. 라이드 체인 로직을 바꿨다면
`js/game.js`의 `runFloors`~`rideDown`을 잘라내 Node에서 스텁(walkTo→setTimeout, DOM 오브젝트 목)으로
돌리는 방식이 확실하다(2026-07-05 리팩토링 때 이 방법으로 상승/하강 전 체인 검증함).
...
- ~~파일 분리~~ ✅ 완료(2026-07-05): style.css + js/{data,engine,world,game}.js
- ~~rideElevator/rideDown 중복~~ ✅ 완료: `rideSequence(cfg)` 공용화
- 남은 부채: setupDay의 태스크 정의를 data.js로 이동, 매직 넘버(좌표)를 층별 레이아웃 상수 객체로, NPC 대사 분기(npcTalk)의 day별 로직을 데이터 주도로.
